...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "POST" && url.pathname === "/api/analyze-image") {
    await handleAnalyzeImageApi(request, response);
    return;
  }

  if (request.method === "POST" && url.pathname === "/api/classify-layout") {
    await handleClassifyLayoutApi(request, response);
    return;
  }

  if (request.method === "POST" && url.pathname === "/api/roast") {
    await handleRoastApi(request, response);
    return;
  }

  if (request.method === "GET" && url.pathname === "/api/debug/prompts") {
  ...
  if (request.method === "GET" && url.pathname === "/api/debug/skills") {
    sendJson(response, 200, {
...
server.listen(port, () => {
  ...
  console.log(`Roast API model: ${siliconFlowModel}`);
  console.log(`Vision API model: ${siliconFlowVisionModel}`);
});

async function handleAnalyzeImageApi(request, response) {
  try {
    ...
    if (!imageUrl) {
      sendJson(response, 400, { error: "imageUrl or imageDataUrl is required." });
      return;
    }
    ...
    if (!completion.ok) {
      await sendUpstreamError(response, completion, "SiliconFlow vision request failed.");
      return;
    }
...
async function handleClassifyLayoutApi(request, response) {
  try {
    requireApiKey(response);
    ...
    if (!photoDescription) {
      sendJson(response, 400, { error: "photoDescription is required." });
      return;
    }
    ...
    if (!completion.ok) {
      await sendUpstreamError(response, completion, "SiliconFlow classification request failed.");
      return;
    }
...
async function handleRoastApi(request, response) {
  try {
    requireApiKey(response);
    ...
    if (!completion.ok) {
      await sendUpstreamError(response, completion, "SiliconFlow API request failed.");
      return;
    }
...
function requireApiKey(response) {
  ...
  sendJson(response, 500, {
    error: "Missing API key. Set SILICONFLOW_API_KEY in .env before running npm run dev."
  });
...
async function sendUpstreamError(response, completion, message) {
...
function sendServerError(response, error) {
  if (error instanceof Error && error.message === "Missing API key.") return;
  sendJson(response, 500, {
